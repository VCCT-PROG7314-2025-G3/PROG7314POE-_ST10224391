...
class ChatRepositoryImpl @Inject constructor(
    ...
    override suspend fun createChat(participantIds: List<String>, offerId: String?): Result<Chat> {
        return try {
            // Check if a chat already exists between these participants for this specific offer
            ...
            existingChatsResult.fold(
                onSuccess = { existingChats ->
                    // Look for a chat with the same participants AND the same offerId
                    ...
                    if (existingChat != null) {
                        // Chat already exists for this specific offer, return it
                        println("Chat already exists for this offer: ${existingChat.id}")
                        Result.success(existingChat)
                    } else {
                        // Create new chat for this offer
                        println("Creating new chat for offer: $offerId")
